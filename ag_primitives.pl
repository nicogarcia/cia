...
%                 sleep(1),
                 ...
                 send_msg(env, i_am(AgID)), %VER
                 write('envió el mensaje i_am'),nl,
                 recv_msg(env, Status),
		 write('recibio mensaje status '), write(Status), nl,
                 !. % Por si acaso, para evitar posible backtracking.

register_me(AgID, Conditions, Status):-
                 write('entro al register me'),nl,
                 connect(env,localhost,8001),
%                 sleep(1),
                 write('pasó el connect'),nl,
		 send_msg(env, starting_conditions(Conditions)),
		 writeln(send_msg(env, starting_conditions(Conditions))),
                 send_msg(env, i_am(AgID)), writeln(send_msg(env, i_am(AgID))),
                 recv_msg(env, Status),
		 write('recibio mensaje status '), write(Status), nl,
                 !. % Por si acaso, para evitar posible backtracking.
